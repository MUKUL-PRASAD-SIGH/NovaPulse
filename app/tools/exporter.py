...
def _export_markdown(data: Dict, filename: str, timestamp: str) -> str:
    ...
    if "entities" in data and data["entities"]:
        e = data["entities"]
        entities_data = e.get("entities", e) if isinstance(e, dict) else {}
        lines.append("\n## 👤 Entity Network\n")
        
        for category in ["people", "organizations", "locations"]:
            items = entities_data.get(category, [])
            if items:
                icon = "👤" if category == "people" else "🏢" if category == "organizations" else "📍"
                lines.append(f"**{icon} {category.capitalize()}:**")
                for item in items[:8]:
                    name = item.get("name", str(item)) if isinstance(item, dict) else str(item)
                    lines.append(f"- {name}")
                lines.append("")
    
    # Social Intelligence (MAS)
    if "social" in data and data["social"]:
        ...
        if reddit:
            ...
            lines.append(f"**Reddit Sentiment:** +{sentiment.get('positive', 0)}% / ~{sentiment.get('neutral', 0)}% / -{sentiment.get('negative', 0)}%")
            lines.append("")
            
            posts = reddit.get("top_posts", reddit.get("posts", []))
            if posts:
    ...
    # Image Intelligence (MAS)
    if "images" in data and data["images"]:
        img = data["images"]
        lines.append("\n## 🖼️ Image Intelligence\n")
        lines.append(f"- **Total Images:** {img.get('total_images', 0)}")
        lines.append(f"- **Analyzed:** {img.get('successful', 0)}")
        lines.append(f"- **With Text:** {img.get('images_with_text', 0)}")
        lines.append(f"- **Manipulation Risks:** {img.get('manipulation_risks', 0)}")
        lines.append("")
